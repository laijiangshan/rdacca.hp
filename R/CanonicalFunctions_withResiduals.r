#Internal function to caculate the Rsquared,adjusted Rsquared, Pseudo-F value, AIC and AICc for cannocial analysis (RDA,db-RDA and CCA)
...
Canonical.Rsq <- function(dv,iv,method="RDA",n.perm=1000){ 
  ...
  if (method=="CCA.vegan"){ # SHOULD WE KEEP THIS???
    cca.res <- cca(dv ~ .,data=data.frame(as.matrix(iv)))
    cca.rsq <- RsquareAdj(cca.res,permutations=n.perm)
    RSq <- cca.rsq$r.squared
    RSqAdj <- cca.rsq$adj.r.squared
    F <- anova(cca.res)$F[1]
  }
